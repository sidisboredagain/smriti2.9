// Vendored from https://github.com/dashersw/liquid-glass-js (MIT License,
// see ./LICENSE) and lightly adapted for use as an ES module inside a Vite
// build: html2canvas is imported instead of assumed to be a global, the
// `Button` cross-reference is duck-typed instead of imported (avoids a
// circular import between container.js/button.js), and a destroy() method
// was added so React can tear an instance's WebGL context + render loop
// down on unmount instead of leaking it forever.
...
class Container {
  ...
  addChild(child) {
    ...
    // If child is a Button-like component, set up nested glass. Duck-typed
    // (rather than `instanceof Button`) so this module never has to import
    // button.js, which would create a circular import (button.js already
    // imports Container from here).
  ...
  /**
   * Stops this instance's rendering, releases its WebGL context, detaches
   * its DOM node, and removes it from Container.instances. Not part of the
   * upstream library - added so a React wrapper can tear an instance down
   * on unmount instead of leaking a live WebGL context + listeners forever.
  ...
  destroy() {
    ...
    // Nested-glass buttons run their own requestAnimationFrame loop and
    // must be stopped explicitly before we drop references to them.
